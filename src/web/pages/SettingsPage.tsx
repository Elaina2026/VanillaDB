...
export const SettingsPage: React.FC = () => {
  ...
  const navTabs = [
    {
      id: 'general' as const,
      label: t('settings.tabGeneral', 'General & Platform'),
      desc: t('settings.descGeneral', 'Instance name, URL & CORS settings'),
      icon: Server,
    },
    {
      id: 'engine' as const,
      label: t('settings.tabEngine', 'SQLite Engine'),
      desc: t('settings.descEngine', 'WAL mode, busy timeout & memory cache'),
      icon: Cpu,
    },
    {
      id: 'backups' as const,
      label: t('settings.tabBackups', 'Backups & Storage'),
      desc: t('settings.descBackups', 'Snapshots, cron schedules & retention'),
      icon: Archive,
    },
    {
      id: 'quotas' as const,
      label: t('settings.tabQuotas', 'User Quotas & Limits'),
      desc: t('settings.descQuotas', 'Database limits & rate limiting'),
      icon: Gauge,
    },
    {
      id: 'alerts' as const,
      label: t('settings.tabAlerts', 'System Alerting & Health'),
      desc: t('settings.descAlerts', 'Webhooks for CPU & RAM alarms'),
      icon: Bell,
    },
    {
      id: 'debug' as const,
      label: t('settings.tabDebug', 'Diagnostics & Debugging'),
      desc: t('settings.descDebug', 'Query logging, stack traces & telemetry'),
      icon: Bug,
    },
    {
      id: 'account' as const,
      label: t('settings.tabAccount', 'My Account & Security'),
      desc: t('settings.descAccount', 'Profile, password, 2FA & passkeys'),
      icon: User,
    },
  ];

  const visibleTabs = isSuperAdminOrAdmin
    ? navTabs
    : navTabs.filter((tab) => tab.id === 'account');

  const currentTabMeta = navTabs.find((t) => t.id === activeTab) || navTabs[0];

  return (
    <div className="flex-1 flex flex-col md:flex-row h-full overflow-hidden w-full select-none bg-background">
      {/* Mobile Topbar for Settings */}
      {isSuperAdminOrAdmin && (
        <div className="md:hidden flex items-center justify-between px-4 py-2.5 border-b border-border bg-card/60 shrink-0">
          <button
            onClick={() => setIsMobileDrawerOpen(true)}
            className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-border bg-card hover:bg-muted text-xs font-semibold text-foreground transition-colors cursor-pointer"
          >
      ...
      {isMobileDrawerOpen && (
        <div className="fixed inset-0 z-50 md:hidden flex">
          ...
          />
          <div className="relative w-72 max-w-[80vw] bg-card border-r border-border h-full flex flex-col z-10 shadow-2xl p-4 space-y-3">
            <div className="flex items-center justify-between pb-3 border-b border-border">
              <div className="flex items-center gap-2">
                <Settings className="w-4 h-4 text-blue-500" />
                <h2 className="text-sm font-bold text-foreground">{t('settings.navHeader', 'Settings')}</h2>
              </div>
              <button
                onClick={() => setIsMobileDrawerOpen(false)}
      ...
      {isSuperAdminOrAdmin && (
        <aside
          className={`hidden md:flex flex-col h-full border-r border-border bg-card/30 transition-all duration-200 shrink-0 select-none ${
            isSidebarOpen ? 'w-64 lg:w-72' : 'w-16'
          }`}
        >
          {/* Sidebar Top: Title + Toggle Button */}
          <div className="p-3 border-b border-border flex items-center justify-between gap-2 shrink-0">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-1.5">
                    <h2 className="text-xs font-bold text-foreground uppercase tracking-wider truncate">
                      {t('settings.navHeader', 'Settings')}
                    </h2>
                    <span className="text-[9px] px-1.5 py-0.2 bg-blue-500/10 text-blue-500 border border-blue-500/20 rounded font-bold uppercase">
            ...
            <button
              ...
              className={`p-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted border border-border/70 transition-colors cursor-pointer shrink-0 ${
                !isSidebarOpen ? 'mx-auto' : ''
              }`}
              title={
                isSidebarOpen
                  ? t('settings.collapseSidebar', 'Thu gọn thanh bên (Collapse)')
                  : t('settings.expandSidebar', 'Mở rộng thanh bên (Expand)')
              }
            >
              {isSidebarOpen ? (
            ...
            {visibleTabs.map((tab) => {
              ...
              if (!isSidebarOpen) {
                return (
                  <button
                    ...
                    title={tab.label}
                    className={`w-full h-10 rounded-xl flex items-center justify-center transition-all cursor-pointer relative ${
                      isActive
              ...
              return (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id as any)}
                  className={`w-full flex items-start gap-2.5 p-2.5 rounded-xl transition-all cursor-pointer text-left ${
                    isActive
                      ? 'bg-blue-500/10 text-blue-500 border border-blue-500/25 shadow-xs font-semibold'
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-1">
                      <span className={`text-xs truncate ${isActive ? 'text-blue-500 font-bold' : 'text-foreground'}`}>
                        {tab.label}
                      </span>
                      {tab.id === 'debug' && current.debug_mode && (
      ...
      <main className="flex-1 flex flex-col h-full overflow-y-auto">
        <div className="pt-6 px-4 md:px-8 pb-16 max-w-5xl mx-auto w-full space-y-6">
          {/* Active Tab Top Header */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-border shrink-0">
            <div>
              <div className="flex items-center gap-2.5">
                ...
                {current.debug_mode && activeTab === 'debug' && (

  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-y-auto p-4 md:p-6 max-w-7xl mx-auto w-full space-y-6 select-none">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-card border border-border rounded-xl p-4 shadow-sm relative overflow-hidden">
          ...
          <div className="flex items-center gap-2 mt-2 text-[11px] text-muted-foreground">
            <span className="text-emerald-500 font-semibold flex items-center">
              <CheckCircle2 className="w-3 h-3 mr-0.5" /> {t('overview.walModeStatus', '100% WAL Mode')}
            </span>
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-4 shadow-sm relative overflow-hidden">
          <div className="flex items-center justify-between text-muted-foreground mb-1.5">
            <span className="text-xs font-semibold uppercase tracking-wider">{t('overview.queries24h', '24h Queries')}</span>
          ...
          <div className="flex items-center gap-2 mt-2 text-[11px] text-muted-foreground">
            <span>{t('overview.avgLatency', 'Avg Latency:')} <strong className="text-foreground">{status?.avgQueryDurationMs ?? 0}ms</strong></span>
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-4 shadow-sm relative overflow-hidden">
          <div className="flex items-center justify-between text-muted-foreground mb-1.5">
            <span className="text-xs font-semibold uppercase tracking-wider">{t('overview.instanceUptime', 'Instance Uptime')}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-card border border-border rounded-xl p-5 shadow-sm space-y-4">
          <h2 className="text-sm font-bold text-foreground flex items-center gap-2">
            <Cpu className="w-4 h-4 text-blue-500" /> {t('overview.hostHardwareTitle', 'Host Hardware & System Resources')}
          </h2>
          ...
          <div className="space-y-3 text-xs">
            ...
            <div className="p-3 bg-muted/40 border border-border rounded-lg space-y-1.5">
              <div className="flex justify-between items-center text-muted-foreground">
                <span className="font-medium">{t('overview.processorCpu', 'Processor (CPU)')}</span>
                <span className="font-mono text-foreground font-semibold">{status?.cpuCount ?? 1} {t('overview.cores', 'Cores')}</span>
              </div>
              <p className="text-[11px] font-mono text-foreground truncate">
                {status?.cpuModel || t('overview.hostCpuArch', 'Host CPU Architecture')}
              </p>
            </div>
            ...
            <div className="p-3 bg-muted/40 border border-border rounded-lg space-y-2">
              <div className="flex justify-between items-center text-muted-foreground">
                <span className="font-medium">{t('overview.osPhysicalRam', 'OS Physical RAM')}</span>
            ...
            <div className="grid grid-cols-2 gap-2 text-[11px]">
              <div className="p-2.5 bg-muted/40 border border-border rounded flex justify-between">
                <span className="text-muted-foreground">{t('overview.nodejsRss', 'Node.js RSS:')}</span>
                <strong className="font-mono">{formatBytes(status?.memoryUsage.rss ?? 0)}</strong>
              </div>
              <div className="p-2.5 bg-muted/40 border border-border rounded flex justify-between">
                <span className="text-muted-foreground">{t('overview.heapUsed', 'Heap Used:')}</span>
        ...
        <div className="bg-card border border-border rounded-xl p-5 shadow-sm space-y-4">
          <h2 className="text-sm font-bold text-foreground flex items-center gap-2">
            <Server className="w-4 h-4 text-emerald-500" /> {t('overview.platformSpecsTitle', 'Platform & Engine Specifications')}
          </h2>

          <div className="space-y-2 text-xs divide-y divide-border">
            <div className="flex justify-between py-2">
              <span className="text-muted-foreground">{t('overview.vdbVersion', 'VanillaDatabase Version')}</span>
              <span className="font-mono font-semibold text-emerald-500">
                v{status?.version || '1.3.2'}
              </span>
            </div>
            <div className="flex justify-between py-2">
              <span className="text-muted-foreground">{t('overview.sqliteCoreEngine', 'SQLite Core Engine')}</span>
              <span className="font-mono font-semibold text-foreground">
                {status?.sqliteVersion || 'SQLite 3.x'} {t('overview.walMode', '(WAL Mode)')}
              </span>
            </div>
            <div className="flex justify-between py-2">
              <span className="text-muted-foreground">{t('overview.nodejsRuntime', 'Node.js JavaScript Runtime')}</span>
              <span className="font-mono font-semibold text-foreground">{status?.nodeVersion || 'v22.x'}</span>
            </div>
            <div className="flex justify-between py-2">
              <span className="text-muted-foreground">{t('overview.hostOs', 'Host Operating System')}</span>
              <span className="font-mono font-medium text-foreground">{status?.platform}</span>
            </div>
            <div className="flex justify-between py-2">
              <span className="text-muted-foreground">{t('overview.activeApiTokens', 'Active API Tokens')}</span>
              <span className="font-mono font-semibold text-blue-500">{status?.totalTokensCount ?? 0}</span>
            </div>
            <div className="flex justify-between py-2">
              <span className="text-muted-foreground">{t('overview.activeWebhookListeners', 'Active Webhook Listeners')}</span>
              <span className="font-mono font-semibold text-purple-500">{status?.activeWebhooksCount ?? 0}</span>
            </div>
            <div className="flex justify-between py-2">
              <span className="text-muted-foreground">{t('overview.errorRate24h', '24h Error Rate')}</span>
      ...
      <div className="bg-card border border-border rounded-xl p-5 shadow-sm space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-sm font-bold text-foreground flex items-center gap-2">
              ...
              {t('overview.quickAccessTitle', 'Tenant Databases Quick Access')}
            </h2>
            <p className="text-xs text-muted-foreground mt-0.5">
              {t('overview.quickAccessDesc', 'Select a database to browse tables, execute SQL, upload media, or manage backups.')}
            </p>
          </div>
          ...
          >
            {t('databases.createDatabase', 'Create Database')}
          </button>
        </div>
        ...
        {databases.length === 0 ? (
          <div className="p-8 border border-dashed border-border rounded-lg text-center space-y-2">
            ...
            <p className="text-xs text-muted-foreground">{t('overview.noDatabasesYet', 'No databases created yet.')}</p>
          </div>
        ) : (

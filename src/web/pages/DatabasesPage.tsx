      ...
      {pendingInvitesCount > 0 && (
        <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-3.5 flex items-center justify-between gap-3 text-xs mb-4">
          <div className="flex items-center gap-2.5 text-blue-500">
            ...
            <span>
              {t('inbox.pendingBanner', 'You have {count} pending database sharing invitations.').replace('{count}', String(pendingInvitesCount))}
            </span>
          </div>
          <a
            href="#/inbox"
          ...
          >
            <span>{t('inbox.viewInbox', 'View Inbox')}</span>
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-12 text-center border border-dashed border-border rounded-2xl bg-card/40 my-6">
          ...
          <h3 className="text-sm font-semibold mb-1 text-foreground">
            {filterType === 'shared' && pendingInvitesCount > 0
              ? t('databases.pendingInvitesTitle', 'Pending invitations waiting in your inbox')
              : search
              ? t('databases.noDatabasesSearch', 'No matching databases found')
              : filterType === 'shared'
              ? t('databases.noSharedDatabases', 'No databases shared with you yet')
              : t('databases.noDatabases', 'No databases found')}
          </h3>
          <p className="text-xs text-muted-foreground max-w-sm mb-5">
            {filterType === 'shared' && pendingInvitesCount > 0
              ? t('databases.pendingInvitesDesc', 'You have {count} database sharing invitations in your inbox. Check your inbox to accept or decline.').replace('{count}', String(pendingInvitesCount))
              : search
              ? t('databases.adjustSearch', 'Try searching by different keywords or clear the filter.')
              : filterType === 'shared'
              ? t('databases.noSharedDatabasesDesc', 'When others share a database with you and you accept from the inbox, it will appear here.')
          ...
          {filterType === 'shared' && pendingInvitesCount > 0 ? (
            ...
            >
              ...
              <span>{t('inbox.viewInbox', 'View Inbox')} ({pendingInvitesCount})</span>
            </a>
          ) : search ? (
            <button
              onClick={() => setSearch('')}
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 mb-2.5">
                    ...
                    {userStats?.rateLimitWarnings?.some((w) => w.databaseId === db.id) && (
                      <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 bg-amber-500/10 text-amber-500 border border-amber-500/30 rounded-md font-semibold">
                        ...
                        {t('databases.rateLimitWarning', 'Rate limit')}
                      </span>
                    )}
                  </div>

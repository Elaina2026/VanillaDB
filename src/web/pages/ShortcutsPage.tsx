...
export const ShortcutsPage: React.FC<{
  ...
  const shortcutGroups = [
    ...
    {
      title: isVi ? 'Trong chi tiết Database (Database Detail Tabs)' : 'Database Detail Tabs',
      shortcuts: [
        { keys: ['1'], label: isVi ? 'Tab Tổng quan Database (Overview)' : 'Overview tab' },
        { keys: ['2'], label: isVi ? 'Tab Bảng & Dữ liệu (Tables Browser)' : 'Tables & Rows Browser' },
        { keys: ['3'], label: isVi ? 'Tab SQL Query Console' : 'SQL Query Console' },
        { keys: ['4'], label: isVi ? 'Tab API Tokens' : 'API Tokens management' },
        { keys: ['5'], label: isVi ? 'Tab Sao lưu & Phục hồi (Backups)' : 'Backups & Snapshots' },
        { keys: ['6'], label: isVi ? 'Tab Tệp & Media Storage' : 'Media Storage & Files' },
        { keys: ['7'], label: isVi ? 'Tab Webhooks' : 'Webhooks stream' },
        { keys: ['8'], label: isVi ? 'Tab Cài đặt Database & Quota' : 'Database Settings & Quota' },
      ],
    },
    {
